...
(define asm-lexer
  (lexer-src-pos
   ...
   ;; Register
   [register
    (lexeme->token/check-delimiter! lexeme input-port token-REGISTER lexeme->symbol)]
   ;; ID
   ...
   [(:: alphabetic (:* (:or alphabetic numeric #\$ #\_ #\.)) ":")
    (lexeme->token/check-delimiter! lexeme input-port token-LABEL lexeme->label)]
   ;;; Binary number
   [(:: (:+ digit2) "b")
    (lexeme->token/check-delimiter! lexeme input-port token-NUMBER lexeme->number2)]
   ;; Decimal number
   [(:+ digit)
    (lexeme->token/check-delimiter! lexeme input-port token-NUMBER lexeme->number10)]
   ;; Hexadecimal number
   [(:: "0x" (:+ digit16))
    (lexeme->token/check-delimiter! lexeme input-port token-NUMBER lexeme->number16)]))
